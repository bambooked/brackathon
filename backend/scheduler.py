"""BTtime のスケジューリングと SSE ブロードキャストを管理する。"""

from datetime import UTC, datetime, timedelta

from apscheduler.schedulers.asyncio import AsyncIOScheduler

EVENT_DURATION_MINUTES = {"time": 30, "fever": 60}

scheduler = AsyncIOScheduler()


async def fire_break_thunder(schedule_id: int, team_name: str) -> None:
    """BTtime を発火: SSE をブロードキャストして DB を fired に更新する。"""
    from models import BreakThunderSchedule
    from sse.manager import manager

    ends_at = datetime.now(UTC) + timedelta(minutes=EVENT_DURATION_MINUTES["time"])
    await manager.broadcast(
        team_name,
        {"type": "bt_time", "ends_at": ends_at.isoformat()},
    )
