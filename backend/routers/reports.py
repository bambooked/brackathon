...
class CreateReportResponse(BaseModel):
    ...
    body: str
    created_at: str
    point_transaction_id: Optional[int] = None  # AI判定でポイントが付与された場合
    points_awarded: int
...
class ReactToReportResponse(BaseModel):
    ...
    reaction: Reaction
    author_point_transaction: Optional[dict] = None  # 日報作成者へのポイント付与
    reactor_point_transaction: Optional[dict] = None  # リアクション者へのポイント付与
    message: str


@router.post("", response_model=CreateReportResponse)
async def create_report(request: CreateReportRequest, authorization: Optional[str] = Header(None)):
    """
