...
async def get_points_status(authorization: str | None = Header(None)):
    """
    現在のポイント状況を取得 (point_accounts)（モック）
    """
    return PointsStatusResponse(
        balance=150, created_at="2026-01-01T00:00:00Z", updated_at="2026-06-06T10:00:00Z"
    )


@router.post("/exchange", response_model=PointExchangeResponse)
async def exchange_points(request: PointExchangeRequest, authorization: str | None = Header(None)):
    """
